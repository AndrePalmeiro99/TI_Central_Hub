export default async function handler(request, response) {
  ...
  try {
    const initRes = await fetch('https://back.cfonety.com.br/gestao/pr-externo/tarefas?limit=1', {
    ...
    const fetchPromises = Array.from({ length: pagesCount }, (_, i) => {
      const page = i + 1;
      return fetch(`https://back.cfonety.com.br/gestao/pr-externo/tarefas?limit=${pageSize}&page=${page}`, {
        headers: { 'x-api-key': key }
      }).then(async res => {
    ...
    console.error('Error fetching paginated PR tasks:', error);
